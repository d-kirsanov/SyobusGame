    ...
    def update(self):
        ...
        if not feet_ray.hit and not head_ray.hit:
            ...
            if (held_keys['left shift'] or held_keys['right shift']) and not(held_keys ['spacebar']):
                ...
                for raycast_number in range(max_raycasts):
                    ray = raycast(self.position + (0,self.height,0) + move_amount * move_percentage, Vec3(0,-1,0),  traverse_target=self.traverse_target, ignore=self.ignore_list)
                    if ray.distance <= self.height+.1:
                        direction = 1
                        if move_percentage == 1:
                            break
                    else:
                        direction = -1
                    move_percentage = move_percentage + direction * move_step
